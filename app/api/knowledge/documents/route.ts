...
export async function POST(request: Request) {
  try {
    ...
    const containerTags = formData.get("containerTags") || '["default"]';
    ...
    // Prepare supermemory upload
    const smForm = new FormData();
    smForm.append("file", file, file.name);
    smForm.append("containerTags", containerTags);
    ...
    const smResult = await response.json();
    return NextResponse.json({ success: true, result: smResult });
  } catch (error) {
    return NextResponse.json({ success: false, message: error instanceof Error ? error.message : "Unknown error" }, { status: 500 });
  }
}
